...
class Bus:
    ...
    @log_on_start(DEBUG, "{self.name:s}: Initiating read by {_name:s}") # type: ignore
    @log_on_error(DEBUG, "{self.name:s}: Error on read by {_name:s}") # type: ignore
    @log_on_end(DEBUG, "{self.name:s}: Finished read by {_name:s}") # type: ignore
    ...
    @log_on_start(DEBUG, "{self.name:s}: Initiating write by {_name:s}") # type: ignore
    @log_on_error(DEBUG, "{self.name:s}: Error on write by {_name:s}") # type: ignore
    @log_on_end(DEBUG, "{self.name:s}: Finished write by {_name:s}") # type: ignore
...
class ConsumerProducer:
    ...
    @log_on_start(DEBUG, "{name:s}: Starting to create consumer-producer") # type: ignore
    @log_on_error(DEBUG, "{name:s}: Encountered an error while creating consumer-producer") # type: ignore
    @log_on_end(DEBUG, "{name:s}: Finished creating consumer-producer") # type: ignore
    def __init__(self,
    ...
    @log_on_start(DEBUG, "{self.name:s}: Starting consumer-producer service") # type: ignore
    @log_on_error(DEBUG, "{self.name:s}: Encountered an error while executing consumer-producer") # type: ignore
    @log_on_end(DEBUG, "{self.name:s}: Closing down consumer-producer service") # type: ignore
    def __call__(self):
    ...
    @log_on_start(DEBUG, "{self.name:s}: Starting collecting bus values into list") # type: ignore
    @log_on_error(DEBUG, "{self.name:s}: Encountered an error while collecting bus values") # type: ignore
    @log_on_end(DEBUG, "{self.name:s}: Finished collecting bus values") # type: ignore
    ...
    # into the buses
    @log_on_start(DEBUG, "{self.name:s}: Starting dealing values into buses") # type: ignore
    @log_on_error(DEBUG, "{self.name:s}: Encountered an error while dealing values into buses") # type: ignore
    @log_on_end(DEBUG, "{self.name:s}: Finished dealing values into buses") # type: ignore
    ...
    @log_on_start(DEBUG, "{self.name:s}: Starting to check termination buses") # type: ignore
    @log_on_error(DEBUG, "{self.name:s}: Encountered an error while checking termination buses") # type: ignore
    @log_on_end(DEBUG, "{self.name:s}: Finished checking termination buses") # type: ignore
...
class Producer(ConsumerProducer):
    ...
    @log_on_start(DEBUG, "{name:s}: Starting to create producer") # type: ignore
    @log_on_error(DEBUG, "{name:s}: Encountered an error while creating producer") # type: ignore
    @log_on_end(DEBUG, "{name:s}: Finished creating producer") # type: ignore
    def __init__(self,
                 producer_function,
                 output_buses,
                 delay=0,
...
class Consumer(ConsumerProducer):
    ...
    @log_on_start(DEBUG, "{name:s}: Starting to create consumer") # type: ignore
    @log_on_error(DEBUG, "{name:s}: Encountered an error while creating consumer") # type: ignore
    @log_on_end(DEBUG, "{name:s}: Finished creating consumer") # type: ignore
    def __init__(self,
                 consumer_function,
                 input_buses,
                 delay=0,
...
class Timer(Producer):
    ...
    @log_on_start(DEBUG, "{name:s}: Starting to create timer") # type: ignore
    @log_on_error(DEBUG, "{name:s}: Encountered an error while creating timer") # type: ignore
    @log_on_end(DEBUG, "{name:s}: Finished creating timer") # type: ignore
    def __init__(self,
    ...
    @log_on_start(DEBUG, "{self.name:s}: Checking current time against starting time") # type: ignore
    @log_on_error(DEBUG, "{self.name:s}: Encountered an error while checking current time against starting time") # type: ignore
    @log_on_end(DEBUG, "{self.name:s}: Finished checking current time against starting time") # type: ignore
    def timer(self):
...
class Printer(Consumer):
    ...
    @log_on_start(DEBUG, "{name:s}: Starting to create printer") # type: ignore
    @log_on_error(DEBUG, "{name:s}: Encountered an error while creating printer") # type: ignore
    @log_on_end(DEBUG, "{name:s}: Finished creating printer") # type: ignore
    def __init__(self,
...
@log_on_start(DEBUG, "runConcurrently: Starting concurrent execution") # type: ignore
@log_on_error(DEBUG, "runConcurrently: Encountered an error during concurrent execution") # type: ignore
@log_on_end(DEBUG, "runConcurrently: Finished concurrent execution") # type: ignore
